...
def pipeline(): # a big method to create a pipline, I would devide it later
...
#store each sheet in a data frame >>>  then creat a new CO2emissions data frame from these data frames (merge them!):
    ...
    CO2emissions_applications = pd.read_excel(CO2emissionsUrl, sheet_name=1)# the second sheet (at 1) define CO2emissions by application
...
#Creating SQLite files
    ...
    sqlite_path = './data/CO2emissionsRenewableEnergy.sqlite'
    engine = create_engine(f'sqlite:///{sqlite_path}')

    print("creating CO2emissions data table ...")
    CO2emissions.to_sql("CO2emissions",engine , if_exists='replace', index=False)
   
     
    print("creating RenewableEnergy data table...")
    RenewableEnergy.to_sql("RenewableEnergy",engine ,if_exists='replace', index=False)
